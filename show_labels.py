...
def save_video_images(vid_path, box_dir, start_from_centre=True):
    box_files = glob.glob(box_dir + '*.txt')
    box_files = sorted(box_files, key=lambda x: int(
        x.rsplit('\\')[-1].rsplit('.')[0]))
    video = cv2.VideoCapture(vid_path)
    ...
    while True:
        ...
        box_name = box_file.rsplit('\\')[-1].rsplit('.')[0]
        image_name = box_name + '.jpg'
        
        cv2.imwrite(os.path.join('images_for_custom', image_name), frame)

if __name__ == '__main__':
    vid_path = 'inputs/media/videos/'
    rain_1_path = vid_path + Rain_1 + '.mp4'
    rain_1_boxes = 'inputs/media/rain_1_yolo/'

    show_video_labels(rain_1_path, rain_1_boxes, start_from_centre=True)

    # # sleep(5)
    # msida_vid = vid_path + msida
    # msida_boxes = "output_coords/videos/Msida/20200323_155250.mp4/"

    # show_video_labels(msida_vid, msida_boxes,start_from_centre=False)

    save_video_images(rain_1_path, rain_1_boxes, start_from_centre=True)
